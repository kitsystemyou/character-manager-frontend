...
interface BasicSkillsEditableTableProps {
  characterSkills: CharacterSkillsType;
  setCharacterSkills: React.Dispatch<React.SetStateAction<CharacterSkillsType>>;
  characterSkillsTableStatus: Skill[];
  setCharacterSkillsTableStatus: React.Dispatch<React.SetStateAction<Skill[]>>;
}
...
const BasicSkillsEditableTable: React.FC<BasicSkillsEditableTableProps> = (props) => {
    ...
    const changeCell = (v: any) => {
        ...
        let sum = status[idx].init_point + status[idx].job_point + status[idx].concern_point + status[idx].grow + status[idx].other
                    - (status[idx] as any)[v.field] + v.value;
        (newValue[idx] as any)[v.field] = v.value;
    ...
    const setTableValue = (newValue: any) => {
        let tableValue: Skill[] = [];
...
export default BasicSkillsEditableTable;