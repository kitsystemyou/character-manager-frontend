...
interface StatusEditableTableProps {
...
const rows: GridRowsProp = fields.STATUS_FIELDS;

const StatusEditableTable: React.FC<StatusEditableTableProps> = (props) => {
    ...
    const setTableValue = (sum: number, field: string, damage_bonus: string, hp: number) => {
        ...
        : field === "POW" ? props.setCharacterStatus({...props.characterStatus, pow:sum, init_san:String(sum*5),luck:String(sum*5),mp:sum,current_san:String(sum*5)})
        ...
        : field === "INT" ? props.setCharacterStatus({...props.characterStatus, int:sum, idea:String(sum*5)})
        : field === "EDU" ? props.setCharacterStatus({...props.characterStatus, edu:sum, knowledge:String(sum*5)})
        : field === "SAN" ? props.setCharacterStatus({...props.characterStatus, current_san:String(sum)})
        : console.log(sum,field)
    }

    const calcDamageBonus = (str: number, siz: number): string =>{
        let damage_bonus ="";
...
export default StatusEditableTable;
